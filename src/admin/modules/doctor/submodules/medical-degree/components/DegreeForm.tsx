...
const DegreeForm: React.FC<CouncilFormProps> = ({ council, onSuccess }) => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const councilData: MedicalDegree = {
        ...data,
        id: council?.id,
        name: data.name ?? " ",
      };
      ...
      await MedicalDegreeService.saveOrUpdate(councilData);
      
      toast({
        title: `Degree ${isEditing ? "updated" : "added"} successfully`,
      });
      ...
      toast({
        title: "Error",
        description: `Failed to ${isEditing ? "update" : "add"} degree. Please try again.`,
        variant: "destructive",
      });
    }
  };
