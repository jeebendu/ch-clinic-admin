...
>(({ className, children, mobileDrawer = false, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 opacity-70 ring-offset-background transition-all hover:opacity-100 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground z-10">
        <X className="h-6 w-6" />
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-center sm:text-left p-6 pb-0 flex-shrink-0",
      className
    )}
    {...props}
  />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 p-6 pt-0 flex-shrink-0",
      className
    )}
    {...props}
  />
