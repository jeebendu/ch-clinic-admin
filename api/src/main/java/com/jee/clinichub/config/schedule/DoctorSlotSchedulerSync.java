...
public class DoctorSlotSchedulerSync {
    ...
    public void releaseSlotToMaster(String targetTenant) {
        ...
        try {
            ...
            List<DoctorBranch> drBranchList = doctorBranchRepo.findAll();
            for (LocalDate date = today; !date.isAfter(endDate); date = date.plusDays(1)) {
                for (DoctorBranch doctorBranch : drBranchList) {
                    log.info("Syncing slots to master for globalDoctorBranchId: {}",
                            doctorBranch.getGlobalDoctorBranchId());
                    doctorSlotSyncService.syncSlotToMaster(doctorBranch.getGlobalDoctorBranchId(),date);
                }
            }
